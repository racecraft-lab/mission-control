...
- **FR-015**: The system MUST provide one checked-in, guarded rollback SQL file for each SQL-changing migration or seed from M53 through M61; column-backed rollback files MUST be replay-safe when already reversed.
- **FR-016**: The system MUST provide a manual rollback procedure that requires a pre-rollback database snapshot, specifies reverse-order execution from M61 to M53, explains SQLite column-rebuild handling for column-backed rollbacks, requires table-backed rollbacks to drop dependent indexes before guarded table drops, limits M59 seed rollback to safe, documented removal of the SPEC-001-created `facility` row only when that row can be identified confidently, and does not require any automated rollback CLI or `Migration.down()` runner.
